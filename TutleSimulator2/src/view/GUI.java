...
import controller.SelectedItem;
...
public class GUI {
	...
	SelectedItem choosenItem1 = new SelectedItem();
	...
	private Button turtleSalatButton;
	...
	public BorderPane addMenu() {
		...
		editorMenu.getItems().addAll(neuItem, öffnenItem, new SeparatorMenuItem(), kompilierenItem, druckenItem,
				new SeparatorMenuItem(), beendenItem);
		...
		terriMenu.getItems().addAll(SpeichernItem, LadenItem, bildSpeichernItem, druckenItem2, größeändernItem,
				new SeparatorMenuItem(), turtlePlazierenMenuItem, salatPlazierenMenuItem, MauerPlazierenMenuItem,
				kachelLöschenMenuItem);
	...
	public ToolBar addToolBar() {
		...
		terrainButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			// Grundlage von für neues Fenster:
		...
		turtleSalatButton.setOnAction(new EventHandler<ActionEvent>() {
		...
		toolbar = new ToolBar(neuButton, öffnenButton, new Separator(), sichernButton, compileButton, new Separator(),
				terrainButton, turtleButton, salatButton, mauerButton, deleteButton, new Separator(), turtleSalatButton,
				turtleLeftButton, turtleMoveButton, turtlePickButton, turtlePutButton, new Separator(), playButton,
				pauseButton, stopButton, new Separator(), tickRateSlider);
	...
	public void editSalatCount() {
		TextInputDialog dialog = new TextInputDialog("Salatwert eingeben");

		dialog.setTitle("Salat ändern");
		dialog.setHeaderText("Neue anzahl der Salate eingeben");
		String input = dialog.getContentText();

		Optional<String> result = dialog.showAndWait();
		try {
			int value = Integer.parseInt(result.get());
			if (value >= 0) {
				terri1.setSalatCounter(value);
			} else {
				Alert errorAlert = new Alert(AlertType.ERROR);
				errorAlert.setHeaderText("Fehlerhafte Eingabe");
				errorAlert.setContentText("Bitte keine negative Zahlen eingeben");
			...
			Alert errorAlert = new Alert(AlertType.ERROR);
			errorAlert.setHeaderText("Fehlerhafte Eingabe");
			errorAlert.setContentText("Bitte nur ganze Zahlen eingeben!");
			errorAlert.showAndWait();
		}
